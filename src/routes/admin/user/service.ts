...
const add = async (user: ISysUser, roleId: string[]) => {
    ...
    try {
        const insertSQL = `
            INSERT INTO sys_user(username,nickname,gender,password,deptId,avatar,mobile,email) VALUES(?,?,?,?,?,?,?,?)
        `;
        const params = [user.username, user.nickname, user.gender, user.password, user.deptId, user.avatar, user.mobile, user.email]
...
const update = async (user: ISysUser, roleIds: string[]) => {
    ...
    await conn.beginTransaction();
    const sql = `
        update sys_user set username = ?,nickname = ?,gender = ?,deptId = ?,avatar = ?,mobile = ?,email = ? where id = ?
    `;
    try {
        //更新用户基本信息
        const [result] = await conn.query(sql, [user.username, user.nickname, user.gender, user.deptId, user.avatar, user.mobile, user.email, user.id]);
...
const del = async (ids: string[]) => {
    ...
    try {
        ...
        const [result] = await conn.query('delete from sys_user where id in (?)', [ids]);
        const resultRet = result as queryResultType
        if (resultRet.affectedRows > 0) {
            // 删除对应的角色
...
const info = async (userId: number) => {
    ...
    const conn = await db.getConnection();
    const [result] = await conn.query('select id,username,nickname,gender,deptId,avatar,mobile,status,email from sys_user where id = ?', [userId]);
    // 查询对应的角色
...
*/
const page = async (page: number, pageSize: number, keywords: string, status: string) => {
    ...
    //查出总数
    const [total] = await conn.query(`select count(id) as total,status from sys_user where concat(nickname,username,mobile) LIKE CONCAT('%', ?, '%') and IF(''=?,TRUE,CAST(status AS SIGNED)=?)`, [toLikeStr(keywords), status, status]);
...
const login = async (username: string, password: string) => {
    const conn = await db.getConnection();
    const [result] = await conn.query('select id,username,nickname,gender,deptId,avatar,mobile,status,email from sys_user where username = ? and password = ?', [username, password]);
    // 释放连接
    await conn.release();
    const rets = result as Array<ISysUser>;
    return rets[0];
...
const changeStatus = async (userId: number, status: number) => { 
    const conn = await db.getConnection();
    const [result] = await conn.query('update sys_user set status = ? where id = ?', [status, userId]);
    const resultRet = result as queryResultType
    if (resultRet.affectedRows > 0) {
        await conn.release()
        return true
...
const resetPassword = async (userId: number, password: string) => { 
    const conn = await db.getConnection();
    const [result] = await conn.query('update sys_user set password = ? where id = ?', [password, userId]);
    const resultRet = result as queryResultType
    if (resultRet.affectedRows > 0) {
        await conn.release()
        return true
